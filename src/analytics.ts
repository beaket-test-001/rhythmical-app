// 계측 래퍼 — Analytics 문서.
//
// v1.0은 이벤트를 수집하지 않는다. 호스팅을 GitHub Pages로 정하면서 호스팅
// 내장 계측(Vercel Web Analytics)을 쓸 수 없게 됐고, 새 도구를 붙이는 것은
// 첫 릴리즈 범위 밖이다.
//
// 근거: 결정 로그 "v0.1 계측 미도입 — track()을 no-op으로 두고 도구는
//       v0.1.1에서 재선정"(2026-09-09 확정, 버전 체계가 1.0.0으로 바뀌기 전
//       표기다). 도구는 다음 패치에서 선정한다. 그때까지 완주율 · 재도전율 등
//       KPI는 측정되지 않는다.
//
// TODO: 다음 패치에서 쿠키 없는 계측 도구를 재선정해 이 함수 안에서만 연결한다.
