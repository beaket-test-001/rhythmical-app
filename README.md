...
**태그는 아래 게이트를 통과했거나, 통과하지 못한 항목이 결정 로그에
명시적으로 감수 기록된 뒤에 붙인다.** 조용히 건너뛰지 않는 것이 요점이다.
...
> v1.0.0은 1번의 실기기 2항목과 3번을 **미검증 상태로 감수하고** 릴리즈했다.
> 근거는 결정 로그의 "iOS · Android 실기기 검증을 MVP 이후로 보류"에 있고,
> CHANGELOG의 알려진 사항에도 밝혀 두었다.
